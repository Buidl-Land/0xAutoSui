...
const getMockAgentData = (agentId: string | string[] | undefined) => {
  ...
  const mockAgents = [
     { id: '1', name: 'Hourly ETH/USDC Arbitrage', status: 'Running', description: 'Checks Uniswap & Sushiswap hourly...', lastModified: Date.now() - 86400000, creator: 'AdminUser', triggerType: 'Scheduled', triggerConfig: [{ type: 'Scheduled', interval: 'hourly' }], mcpConfig: [{ name: 'Price Feed MCP', id: 'mcp1' }, { name: 'DEX Swap MCP', id: 'mcp2' }], logs: [{ id: 'log1', timestamp: Date.now() - 3600000, message: 'Run 1: Success' }, { id: 'log2', timestamp: Date.now(), message: 'Run 2: Success' }], settings: { systemPrompt: 'Act as an arbitrage bot.', icon: null } },
     { id: '2', name: 'KOL Tweet Trader (DegenMode)', status: 'Running', description: 'Monitors @CryptoKOL tweets...', lastModified: Date.now() - 172800000, creator: 'TraderX', triggerType: 'Event: Social', triggerConfig: [{ type: 'Event: Social', keyword: '#DegenToken', user: '@CryptoKOL' }], mcpConfig: [{ name: 'Social Feed MCP', id: 'mcp3' }, { name: 'DEX Swap MCP', id: 'mcp2' }], logs: [{ id: 'log3', timestamp: Date.now() - 7200000, message: 'Run 1: Bought 100 DT' }, { id: 'log4', timestamp: Date.now(), message: 'Run 2: No trigger' }], settings: { systemPrompt: 'Act as a degen trader.', icon: null } },
     { id: '3', name: 'BTC Price Dip Buyer', status: 'Stopped', description: 'Triggers when BTC price drops 5%...', lastModified: Date.now(), creator: 'AdminUser', triggerType: 'Event: Price', triggerConfig: [{ type: 'Event: Price', asset: 'BTC', threshold: '-5%' }], mcpConfig: [{ name: 'Price Feed MCP', id: 'mcp1' }, { name: 'CEX Trade MCP', id: 'mcp4' }], logs: [{ id: 'log5', timestamp: Date.now(), message: 'Run 1: Stopped manually' }], settings: { systemPrompt: 'Buy the dip.', icon: null } },
     { id: '4', name: 'New CA Sniper (BNB Chain)', status: 'Error', description: 'Monitors new contract deployments...', lastModified: Date.now() - 3600000, creator: 'SniperJoe', triggerType: 'Event: Chain', triggerConfig: [{ type: 'Event: Chain', event: 'ContractDeployed', chain: 'BNB' }], mcpConfig: [{ name: 'Chain Event MCP', id: 'mcp5' }, { name: 'DEX Swap MCP', id: 'mcp2' }], logs: [{ id: 'log6', timestamp: Date.now(), message: 'Run 1: Error - Insufficient funds' }], settings: { systemPrompt: 'Snipe new tokens.', icon: null } },
  ];
...
const AgentDetailPage = () => {
  ...
  const [iconPreviewUrl, setIconPreviewUrl] = useState<string | null>(agent?.settings?.icon || null); // Initialize with existing icon if available
                ...
                <div key={mcp.id} className="card bg-base-200 shadow">
                  <div className="card-body p-4">
                    ...
                    <div className="card-actions justify-end mt-2">
                      ...
                      <button className="btn btn-ghost btn-xs" aria-label={`Edit MCP ${mcp.name}`} onClick={() => handleOpenMcpModal(mcp)}><PencilSquareIcon className="h-4 w-4" /></button>
                      <button className="btn btn-ghost btn-xs text-error" aria-label={`Delete MCP ${mcp.name}`} onClick={() => alert(`Mock Delete MCP: ${mcp.name}`)}><TrashIcon className="h-4 w-4" /></button> {/* Keep delete as alert for now */}
                    </div>
                  </div>
                </div>
              ))}
                ...
                <div key={index} className="card bg-base-200 shadow">
                  <div className="card-body p-4">
                    ...
                    <div className="card-actions justify-end mt-2">
                      ...
                      <button className="btn btn-ghost btn-xs text-error" aria-label={`Delete Trigger ${trigger.type}`} onClick={() => alert(`Mock Delete Trigger: ${trigger.type}`)}><TrashIcon className="h-4 w-4" /></button> {/* Keep delete as alert for now */}
                    </div>
                  </div>
                </div>
              ))}
                ...
                <div key={log.id} className="card card-compact bg-base-200 shadow">
                  <div className="card-body flex-row justify-between items-center p-3">
                    ...
                    <div className="card-actions">
                      ...
                      <button className="btn btn-ghost btn-xs text-error" aria-label={`Delete Log ${log.id}`} onClick={() => alert(`Mock Delete Log: ${log.id}`)}><TrashIcon className="h-4 w-4" /></button> {/* Keep delete as alert */}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <dialog id="agent_edit_modal" className="modal" ref={agentModalRef}> {/* Updated ref */}
        <div className="modal-box w-11/12 max-w-2xl">
          ...
          <div className="form-control mb-4">
            ...
            <input type="text" defaultValue={agent?.name ?? ''} className="input input-bordered w-full" />
          </div>
              ...
              <div className="flex items-center gap-4">
                  ...
                  <div className="avatar">
                      <div className="w-16 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
                          <img src={iconPreviewUrl || '/logo.png'} alt="Agent Icon Preview" /> {/* Fallback to default logo */}
                      </div>
                  </div>
                  {/* Upload Button */}
          ...
          {editingMcp && (
            <div>
              <div className="form-control mb-4">
                ...
                <input type="text" defaultValue={editingMcp.name} className="input input-bordered w-full" />
              </div>
              {/* Enhanced MCP Fields */}
              <div className="form-control mb-4">
